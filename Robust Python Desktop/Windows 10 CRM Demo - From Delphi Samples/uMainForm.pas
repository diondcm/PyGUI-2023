...
uses
  ...
  PythonEngine, PythonSource, Vcl.DBActns, REST.Types, REST.Client,
...
type
  TMainForm = class(TForm)
    ...
    procedure VirtualImage3Click(Sender: TObject);
  private
    FRanOnce: Boolean;
...
uses uDataMod, uLeads, uDraftProposal, uProposal, Form.Login;
...
begin
  ...
  try
    ...
    Application.MessageBox(PChar(FResult.Text), 'Response', MB_OK + MB_DEFBUTTON1 + MB_ICONINFORMATION);
...
procedure TMainForm.VirtualImage3Click(Sender: TObject);
begin
  var frm := TfrmLogin.Create(nil);
  frm.ShowModal;
  frm.Free;
end;
...
procedure TMainForm.PythonInputOutputSendUniData(Sender: TObject;
  const Data: string);
{
...
begin
  try
    ...
    begin
      // Some exception in Python Integration
    end;
    on E: EListError do
    begin
      // Some list exception
    ...
    on E: Exception do
    begin
      // Deal with some general exception
    end;
  end;
end;
